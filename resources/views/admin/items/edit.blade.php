            ...
            <form action="{{ route('items.update',$item->item_id) }}" method="POST"  enctype="multipart/form-data">
                ...
                <div class="row g-3">
                    ...
                    <div class="col-md-6">
                        ...
                        <input type="number" name="item_price" class="form-control" value="{{$item->item_price}}" placeholder="₱0.00">
                    </div>
                    <div class="col-md-6">
                        <label class="form-label">Quantity</label>
                        <input type="number" name="qty" class="form-control" value="{{$item->qty}}" min="1" step="1">
                    </div>
                    <div class="col-md-6">
                        <label class="form-label">Category</label>
                        <select name="category" class="form-select">
                            @foreach ($category as $ctgy)
                                <option value="{{ $ctgy->category_id }}" {{ ($item->category_id==$ctgy->category_id) ? 'selected' : '';}}>{{ $ctgy->category_name }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="col-12">
                        <label class="form-label">Item Description</label>
                        <textarea name="item_desc" class="form-control" rows="4" placeholder="Describe the item...">{{$item->description}}</textarea>
                    </div>
                    <div class="col-12">
